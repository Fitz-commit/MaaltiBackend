...
public class Main {
    ...
    public static void main(String[] args) throws IOException, URISyntaxException {
        URI abc = new URI("https://api.instagram.com/oauth/authorize?client_id=594821241888095&redirect_uri=https://insta-test-maalti.herokuapp.com/auth/&scope=user_profile,user_media&response_type=code");

        if (Desktop.isDesktopSupported() && Desktop.getDesktop().isSupported(Desktop.Action.BROWSE)) {
            Desktop.getDesktop().browse(abc);
        }
//Wenn diese URI aufgerufen wird dann bei der @Getxyz die Query abgreifen ?
